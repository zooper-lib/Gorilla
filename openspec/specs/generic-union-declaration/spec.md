...
Defines how the Gorilla source generator handles `[DiscriminatedUnion]` types that declare type parameters. This covers accepting type parameters on a union, capturing type parameter names and constraint clauses as cache-safe strings, appending the type argument list to emitted self-references, reopening generic containing types, naming the `Match` result type parameter `TResult`, how a union names itself in diagnostics versus runtime messages, and the deliberate omissions — no companion static factory class and no variance support — with all generated output compiling warning-clean.
