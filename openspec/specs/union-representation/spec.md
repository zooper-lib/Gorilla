...
Defines the shape a `[DiscriminatedUnion]` type is generated as: an `abstract` base type whose variants are `sealed` nested subtypes deriving from it, produced by a single emit path with no wrapper representation and no third-party base class. This covers the class/record choice, variant naming and payload exposure, the private constructor that closes the hierarchy, what equality and `ToString` are left to the language, and the rejection of struct unions.
