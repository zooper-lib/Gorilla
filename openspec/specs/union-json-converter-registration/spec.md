# union-json-converter-registration

## Purpose

Defines how generated JSON converters for `[DiscriminatedUnion]` types are registered and discovered by both serializers. This covers emitting the System.Text.Json converter attribute on variant types, claiming the whole union hierarchy through `CanConvert`, registering a generic union's converters through a non-generic System.Text.Json factory and a non-generic Newtonsoft delegating shim, choosing the scope a converter is emitted into, repeating constraint clauses on the converter, and suppressing the Native AOT warning at runtime reflection sites.
