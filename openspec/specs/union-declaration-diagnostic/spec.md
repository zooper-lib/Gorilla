# union-declaration-diagnostic

## Purpose

Defines how a malformed union *declaration* is reported rather than silently corrected: `ZGOR003` for a `[DiscriminatedUnion]` type that is not `abstract`, its registration in analyzer release tracking, and the Roslyn code fix that rewrites the declaration's modifiers. The generator does not inject `abstract` into its own partial half, so the declaration the user reads never disagrees with the type that is produced.
